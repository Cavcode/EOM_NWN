...
namespace EOM.Game.Server.Feature.GuiDefinition.ViewModel
{
    public class HoloNetViewModel : GuiViewModelBase<HoloNetViewModel, GuiPayloadBase>
    {
        ...
        public Action OnClickSubmit() => () =>
        {
            ...
            {
                var url = Environment.GetEnvironmentVariable("EOM_HOLONET_WEBHOOK_URL");

                if (string.IsNullOrWhiteSpace(url))
                {
                    SendMessageToPC(Player, ColorToken.Red("ERROR: Unable to send the HoloNet broadcast because server admin has not specified the 'EOM_HOLONET_WEBHOOK_URL' environment variable."));
                    return;
                }
