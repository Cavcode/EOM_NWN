...
{
    public class MightySwingsAbilityDefinition : IAbilityListDefinition
    {
        public Dictionary<FeatType, AbilityDetail> BuildAbilities()
        {
            var builder = new AbilityBuilder();
            MightySwings(builder);
        ...
        private static void ImpactAction(uint activator, uint target, int level, Location targetLocation)
        {
            ...
            DelayCommand(1.0f, () => ApplyEffectToObject(DurationType.Instant, EffectVisualEffect(VisualEffect.Vfx_Imp_MightySwings, false, 1.0f), activator)); 
            var creature = GetFirstObjectInShape(Shape.SpellCone, RadiusSize.Huge, targetLocation, true, ObjectType.Creature, GetPosition(activator));
                ...
                while (GetIsObjectValid(creature))
                {
                    if (GetIsReactionTypeFriendly(activator) == 0 && creature != activator)
                    {
                        ...
                        var damage = Combat.CalculateDamage(
                            ...
                            defenderStat,
                            0);
                        Console.Write(damage.ToString() + " damagess");

                        var dTarget = creature;
                        DelayCommand(0.1f,
                            () =>
                            {
                                ApplyEffectToObject(DurationType.Instant, EffectVisualEffect(VisualEffect.Vfx_Sf_Elec), dTarget);
                    ...
                    creature = GetNextObjectInShape(Shape.SpellCone, RadiusSize.Huge, targetLocation, true, ObjectType.Creature, GetPosition(activator));
                }
            });
            ...
            ApplyEffectToObject(DurationType.Instant, EffectVisualEffect(VisualEffect.Vfx_Sf_AxeSwing2, false, 1.0f),
                activator);
            AssignCommand(activator, () => ActionPlayAnimation(Animation.ThinkingMan));
            DelayCommand(0.6f, () => ApplyEffectToObject(DurationType.Instant, EffectVisualEffect(VisualEffect.Vfx_Sf_ShockWave, false, 1.0f), activator));
        ...
        private static void MightySwings(AbilityBuilder builder)
        {
            builder.Create(FeatType.MightySwings, PerkType.MightySwings)
                .Name("Mighty Swings")
                .Level(1)
                .HasRecastDelay(RecastGroup.MightySwings, 2f)
                .RequirementMagick(3)
