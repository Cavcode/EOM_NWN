...
namespace EOM.Game.Server.Core.NWNX
{
    public class ItemPlugin
    {
        ...
        public static void SetItemAppearance(uint oItem, int nType, int nIndex, int nValue, int bUpdateCreatureAppearance = 0)
        {
        ...
        public static int MoveTo(uint oItem, uint oTarget, int bHideAllFeedback = 0)
        {
        ...
        public static void SetMinEquipLevelModifier(uint oItem, int nModifier, int bPersist = 1)
        {
        ...
        /// @warning if Persistence is FALSE, or not renabled, beware characters may trigger ELC logging in with now-invalid ItemLevelRestrictions equipped.
        public static void SetMinEquipLevelOverride(uint oItem, int nOverride, int bPersist = 1)
        {
            NWNXPushInt(bPersist);
